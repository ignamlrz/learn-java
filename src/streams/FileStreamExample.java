package streams;

import java.io.*;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.Random;

/**
 * The {@link FileStreamExample} class contain a set of implementation examples
 * using I/O File streams
 *
 * @author Tim Buchalka
 */
public class FileStreamExample {

    public static void main(String[] args) throws IOException {
        String fileNameExample_1 = "characterData.txt";
        writeRandomNumbersToFile(fileNameExample_1);
        List<Integer> randomList =
                readRandomNumbersFromFile(fileNameExample_1);

        String fileNameExample_2 = "binaryIOData.bin";
        writeRandomNumbersDataStream(fileNameExample_2, randomList);
        List<Integer> dataList =
                readRandomNumbersDataStream(fileNameExample_2);
        System.out.println("Are lists equal? " + randomList.equals(dataList));
    }

    /**
     * Method which use {@link FileWriter} to print 2500 random numbers on a
     * file
     *
     * @param fileName Name of the file
     * @throws IOException I/O Exception
     */
    private static void writeRandomNumbersToFile(String fileName)
            throws IOException {
        Random r = new Random();
        String numbersList;

        // Use try with resources to create output character stream, FileWriter
        // dumping a set of numbers to the file.
        try (FileWriter outputStream = new FileWriter(fileName)) {
            // Create 100 sets of random numbers
            for (int i = 0; i < 100; i++) {

                // Use random generator to create a set of 25 numbers between 1 and 100
                numbersList = r.ints(25, 1, 100)
                        // Map to string
                        .mapToObj(String::valueOf)
                        // Create a single string from all 25 numbers delimited by space
                        .reduce("", (string, element) -> String.join(" ", string, element));
                // Dump the string to the output file with a carriage return
                outputStream.write(numbersList + "\n");
            }
        }
    }

    /**
     * Method which uses a {@link FileReader} wrapped in a
     * {@link BufferedReader}
     *
     * @param fileName Name of the file
     * @return list of random numbers read
     * @throws IOException I/O exception
     */
    private static List<Integer> readRandomNumbersFromFile(String fileName)
            throws IOException {

        List<Integer> numbers = new ArrayList<>();

        // Using BufferedReader wrapping a FileReader.  More efficient and provides
        // readLine method
        try (BufferedReader inStream =
                     new BufferedReader(new FileReader(fileName))) {
            String line;
            // Read each line.
            while ((line = inStream.readLine()) != null) {
                // Split each line by white space and add numbers to a list
                Arrays.asList(line.split("\\s"))
                        .forEach((s) -> {
                            try {
                                numbers.add(Integer.parseInt(s));
                            } catch (Exception e) {
                                // ignore junk;
                            }
                        });

            }
        }
        System.out.println("Total number of numbers retrieved = " +
                numbers.size());
        return numbers;
    }

    // This method uses a DataInputStream to write numbers in a list

    /**
     * Method which use a {@link FileOutputStream} wrapped on a
     * {@link BufferedOutputStream}, which is wrapped on a
     * {@link DataOutputStream} class, for print a random numbers on a file
     *
     * @param fileName Name of the file
     * @param list     List of random numbers to write
     * @throws IOException I/O Exception
     */
    private static void writeRandomNumbersDataStream(String fileName,
                                                     List<Integer> list)
            throws IOException {

        // Use try with resources to create output character stream
        // dumping a set of numbers to the file.
        try (DataOutputStream outputStream =
                     new DataOutputStream(new BufferedOutputStream(
                             new FileOutputStream(fileName)))) {
            list.forEach((s) -> {
                try {
                    outputStream.writeInt(s);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            });
        }
    }

    /**
     * Method which use a {@link FileInputStream} wrapped on a
     * {@link BufferedInputStream}, which is wrapped on a
     * {@link DataInputStream} class, for read a random numbers from file
     *
     * @param fileName File of the name
     * @return a list of integers
     * @throws IOException I/O exception
     */
    private static List<Integer> readRandomNumbersDataStream(String fileName)
            throws IOException {

        List<Integer> numbers = new ArrayList<>();

        // Using DataInputStream, passing a BufferedInputStream wrapping
        // a FileInputStream.
        try (DataInputStream inStream = new DataInputStream(new
                BufferedInputStream(new FileInputStream(fileName)))) {
            try {
                while (true) {
                    // Note that you can use readInt
                    numbers.add(inStream.readInt());
                }
            } catch (EOFException e) {
                // Ignore, end of file
            }
        }
        System.out.println("Total number of numbers retrieved = " +
                numbers.size());
        return numbers;
    }
}
